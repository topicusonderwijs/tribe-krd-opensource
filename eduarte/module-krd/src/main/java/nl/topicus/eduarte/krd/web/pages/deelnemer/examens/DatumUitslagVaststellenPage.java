...
/**
 * Pagina waarop een datumuitslag aangegeven kan worden.
 */
@PageInfo(title = "Examens", menu = "Deelnemer -> Examens -> Actie overzicht -> [Actie]")
@InPrincipal(DeelnemerExamensCollectief.class)
public class DatumUitslagVaststellenPage extends SecurePage implements IEditPage
{
	...
	public DatumUitslagVaststellenPage(KwalificatieModel kwalificatieModel)
	{
		super(CoreMainMenuItem.Deelnemer);
		this.kwalificatieModel = kwalificatieModel;
		form = new Form<Void>("form");
		form.add(new RequiredDatumField("datumUitslag", new PropertyModel<Date>(kwalificatieModel,
			"datumUitslag")));
		add(form);
		createComponents();
	}
